...
class MedicalRecordCreationDiagnosisExtra extends Simulation {
	...
	object MedicalRecordFormPositive {
	  val feederPositive = csv("medicalRecordCreationPositive.csv")
	...
	object MedicalRecordFormNegative {
	  val feederNegative = csv("medicalRecordCreationNegative.csv")
	...
	setUp(medicalRecordCreationPositiveScn.inject(rampUsers(250) during (100 seconds)), medicalRecordCreationNegativeScn.inject(rampUsers(250) during (100 seconds))).protocols(httpProtocol)
	.assertions(
